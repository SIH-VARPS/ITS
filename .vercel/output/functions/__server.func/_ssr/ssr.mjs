...
function predictDelay(features, targetHaltIndex) {
  ...
  const histComponent =
    (features.priorHaltDelays.length ? histSum / features.priorHaltDelays.length : 0) *
    0.4 *
    Math.max(0, targetIdx - lastIdx);
  let runComponent = 0;
  ...
  let confidence =
    0.62 + Math.min(1, features.priorHaltDelays.length / 4) * 0.2 - Math.min(0.35, horizon * 0.05);
...
function buildFeatures(train, state, weather, corridorCongestion) {
  ...
  const wTable = ["clear", "clear", "clear", "clear", "rain", "fog", "wind"];
...
function forecastEtaAtHalt(train, haltIndex, live, now) {
  const delayAtTarget = predictDelay(
    buildFeatures(train, {
      ...live,
      date: now,
    }),
...
function computeLiveStatus(train, now) {
  ...
  if (nextHalt)
    if (clamped <= lastHalt.dep) state = state === "running" ? "halted" : state;
    else {
...
var RailBackendService = class {
  ...
  static getConnectingImpact(incomingTrainNo, connectingTrainNo, transferStationCode) {
    ...
    const haltForecast = computeLiveStatus(inTrain, /* @__PURE__ */ new Date()).haltStatus.find(
...
async function getServerEntry() {
  if (!serverEntryPromise)
    serverEntryPromise = import("./server-ICti-Pk3.mjs").then((m) => m.default ?? m);
...
export {
  ...
  stationMap as l,
  confidenceTier as n,
  historicalDelayAt as o,
  renderErrorPage as p,
  delayLabel as r,
  DELAY_REASONS as s,
  computeLiveStatus as t,
  findTrains as u,
};
